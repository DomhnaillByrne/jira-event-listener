        	...
        	for (int i = 0; i < changeItems.size(); i++ ){
        		...
        		if (item.containsValue("priority")){
        			try {
                		...
                		email.setBody("You've updated Priority on " + baseurl + "/browse/" + issue.getKey() + ", Please edit the autogenerated comment with a reason.");
                		try {
        					mailServer.send(email);
        					ComponentAccessor.getCommentManager().create(issue, author, "PRIORITY UPDATED TO BLOCKER, PLEASE EDIT ME", false);
